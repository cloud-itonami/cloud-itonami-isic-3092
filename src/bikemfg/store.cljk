(ns bikemfg.store
  "SSoT for the bicycle-and-invalid-carriage plant-operations
  ...
  Scope note: like its siblings (`cloud-itonami-isic-2211`'s own
  `tyremfg.store`, `cloud-itonami-isic-2013`'s own `resinmfg.store`,
  `cloud-itonami-isic-2220`'s own `plasticsmfg.store`), this build
  ...
  self-contained (no external bicycle/invalid-carriage-manufacturing
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             product-category/weight-capacity/quantity/
                             weld-defect-rate record. `:verified?`
                             marks whether the batch's own claims have
                             actually been QC-inspected (never inferred
                             from a routine intake patch); `:registered?`
    ...
    - `maintenance`       -- a scheduled welding/assembly/test-bench
                             ...
                             of equipment (`bikemfg.registry`'s
    ...
    - `shipments`         -- a proposed outbound bicycle/invalid-
                             carriage shipment DRAFT (`bikemfg.registry`'s
                             `register-shipment`).
  ...
  coordinator needs."
  (:require [bikemfg.registry :as registry]))
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (bikemfg.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (bikemfg.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :product-category :road-bicycle
                :frame-size "56cm" :weight-capacity-kg 120.0
                :quantity-units 500.0 :weld-defect-rate-percent 0.8
                :verified? true :registered? true
                :shipped-units 100.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :product-category :power-wheelchair
                :frame-size "adult-standard" :weight-capacity-kg 180.0
                :quantity-units 80.0 :weld-defect-rate-percent 0.3
                :verified? true :registered? true
                :shipped-units 75.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :product-category :mountain-bicycle
                :frame-size "48cm" :weight-capacity-kg 140.0
...
(defn- sample-equipment []
  {"welder-001" {:id "welder-001" :kind :frame-welding-robot
                 :verified? true :registered? true
                 :last-maintenance-date "2026-05-01"}
   "testbench-002" {:id "testbench-002" :kind :structural-test-bench
                     :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `bikemfg.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `bikemfg.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  set -- one verified+registered bicycle batch with shipping headroom
  (schedulable), one verified+registered wheelchair batch that is
  ...
  maintenance), one UNVERIFIED/unregistered structural-test-bench unit
