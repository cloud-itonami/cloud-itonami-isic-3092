(ns bikemfg.advisor
  "BikeAdvisor -- the *contained intelligence node* for the bicycle-
  and-invalid-carriage plant-operations coordination actor.

  It normalizes production-batch patches (product-category/weight-
  capacity/quantity/weld-defect-rate), drafts a welding/assembly/
  ...
  bicycle/invalid-carriage shipment coordination proposal against a
  ...
  a committed record and NEVER a real welding/assembly/test-bench-
  equipment actuation, freight dispatch, or ISO 4210/ISO 7176
  bicycle-or-wheelchair safety certification -- see README `What this
  actor does NOT do`. Every output is censored downstream by
  `bikemfg.governor` before anything touches the SSoT.
  ...
  contract every caller of this actor agrees to) -- `bikemfg.governor`
...
(defn- log-production-batch
  ...
  category, weight-capacity, quantity, weld-defect-rate or
...
(defn- schedule-maintenance
  ...
  `bikemfg.governor` NEVER trusts this report -- it independently
...
(defn- flag-safety-concern
  "Draft a frame-weld-defect/brake-safety/structural-integrity
  ...
  technicality). See `bikemfg.phase`: no phase ever adds this op to a
  phase's `:auto` set; `bikemfg.governor` also always escalates on
...
(defn- coordinate-shipment
  "Draft an outbound bicycle/invalid-carriage shipment coordination
  ...
  `bikemfg.governor` NEVER trusts it: it independently recomputes
...
(def ^:private system-prompt
  (str "あなたは自転車・介護用車いす製造工場プラント運用コーディネーターの"
       "助言者です。"
       ...
       "ISO 4210/ISO 7176安全認証を自己発行する提案をしてはいけません。"
       "出荷数量を偽って報告してはいけません。"))
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `bikemfg.governor`
