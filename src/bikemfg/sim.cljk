(ns bikemfg.sim
  ...
  override), a proposal that tries to self-issue an ISO 4210/ISO 7176
  bicycle-or-wheelchair safety certification (permanently blocked, no
  override), a double-schedule of the same maintenance window, a
  production-batch patch with a fabricated product category, a
  production-batch patch with an implausible weight-capacity, and a
  production-batch patch with an implausible weld-defect-rate
  reading.
...
(defn -main [& _args]
  (let [db (-> (store/mem-store) (store/sample-data!))
    ...
    (println "== schedule-maintenance mnt-1 on welder-001 (verified, registered, frame-welding robot -- escalates, approve) ==")
    ...
    (println "== flag-safety-concern concern-1 on welder-001 (always escalates -- approve) ==")
    ...
    (println "== schedule-maintenance mnt-3 on welder-001 with :actuate-equipment? true -> HARD hold, PERMANENT, never reaches a human ==")
    ...
    (println "== log-production-batch batch-001 attempting to self-issue an ISO 4210/ISO 7176 certification -> HARD hold, PERMANENT ==")
