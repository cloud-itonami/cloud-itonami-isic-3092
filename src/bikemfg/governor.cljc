(ns bikemfg.governor
  "Bicycle & Mobility Device Plant Operations Governor -- the
  independent compliance layer that earns the BikeAdvisor the right
  to commit. The advisor has no notion of whether a piece of
  ...
  merely draft-schedule) welding/assembly/test-bench equipment,
  whether a proposal secretly tries to self-issue an ISO 4210 (bicycle
  safety) / ISO 7176 (wheelchair safety) safety CERTIFICATION (an
  authority this actor never holds), whether a shipment proposal's
  own claimed quantity would blow through the batch's own logged
  production quantity, or when an act stops being a coordination
  proposal and becomes direct welding/assembly/test-bench-equipment
  control, so this MUST be a separate system able to *reject* a
  proposal and fall back to HOLD.

  `:itonami.blueprint/governor` is
  `:bicycle-mobility-plant-operations-governor` (see
  ...
  gate (SOFT -- asks a human to look, and the human may approve):
    ...
    5. Certification-authority
       blocked                     -- ANY proposal (any op) whose own
                                       ...
                                       attempting to self-issue an ISO
                                       4210 (bicycle) / ISO 7176
                                       (wheelchair) safety
...
(defn- certification-authority-blocked-violations
  ...
  to self-issue an ISO 4210 (bicycle) / ISO 7176 (wheelchair) safety
  ...
  (let [payload (or (:value proposal) (:patch proposal))]
    (when (true? (:issue-certification? payload))
      [{:rule :certification-authority-blocked
        :detail "ISO 4210/ISO 7176安全認証の自己発行提案は恒久的に禁止 -- 認証機関の専権事項"}])))
...
(defn check
  "Censors a BikeAdvisor proposal against the governor rules.
