(ns bikemfg.operation
  "BikeOperationActor -- one plant-operations coordination request =
  one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (BikeAdvisor) is sealed into a single node (:advise);
  its proposal is ALWAYS routed through the Bicycle & Mobility Device
...
(defn build
  "Compiles a BikeOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `bikemfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; BikeAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Bicycle & Mobility Device Plant Operations Governor -- independent censor.
      (g/add-node :govern
