(ns bikemfg.registry
  "Pure-function domain logic for the bicycle-and-invalid-carriage
  (wheelchair/mobility-aid) plant-operations coordination actor --
  equipment/batch verification, shipment-quantity recompute,
  product-category validation, weight-capacity plausibility
  validation, weld-defect-rate plausibility validation, and draft
  ...
  never the act of issuing an ISO 4210 (bicycle safety) / ISO 7176
  (wheelchair safety) certification (this actor NEVER does any of
  those -- see README `What this actor does NOT do`).

  SCOPE: ISIC 3092 covers manufacture of bicycles and invalid
  carriages -- frame-welding, assembly, and test-bench (structural/
  brake) inspection lines producing finished bicycles (road/mountain/
  city/cargo/electric-assist/BMX/kids) and invalid carriages (manual
  wheelchairs, power wheelchairs, mobility scooters). This actor
  ...
  the certification body that issues bicycle/wheelchair safety
  approval marks.")
...
(def valid-product-categories
  ...
  may declare -- spans both bicycles and invalid carriages (wheelchairs/
  mobility aids), the two product families ISIC 3092 covers. Anything
  else is a fabricated/unrecognized product category -- the governor
  HARD-holds rather than let an invented category pass through."
  #{:road-bicycle :mountain-bicycle :city-bicycle :cargo-bicycle
    :electric-assist-bicycle :bmx-bicycle :kids-bicycle
    :manual-wheelchair :power-wheelchair :mobility-scooter})

(def weight-capacity-min-kg
  "Physical floor for a bicycle/invalid-carriage's own rated maximum
  occupant/load weight capacity."
  0)

(def weight-capacity-max-kg
  "Physical ceiling for a bicycle/invalid-carriage's own rated maximum
  occupant/load weight capacity (spans light kids' bicycles through
  heavy-duty bariatric power wheelchairs and cargo bicycles). A
  reading above this is an implausible/fabricated rating, not a real
  standardized weight-capacity spec."
  300)
...
(defn batch-verified?
  ...
  (i.e. its product-category/weight-capacity/quantity/weld-defect-rate
...
(defn product-category-valid?
  ...
  values (bicycles or invalid carriages)? nil/blank is treated as
  invalid (a production-batch patch must declare a real product
  category, not omit it silently)."
  [product-category]
  (contains? valid-product-categories product-category))

(defn weight-capacity-valid?
  "Is `weight-capacity-kg` a physically plausible rated maximum
  occupant/load weight capacity? Rejects nil, non-numbers, negative
  values, and values beyond `weight-capacity-max-kg` -- a fabricated
  or sensor-error reading, never let through as a real product-rating
  fact."
  [weight-capacity-kg]
  (and (number? weight-capacity-kg)
       (>= (double weight-capacity-kg) (double weight-capacity-min-kg))
       (<= (double weight-capacity-kg) (double weight-capacity-max-kg))))
...
(defn- unsigned-certificate
  ...
  actor's. And NEVER an ISO 4210 / ISO 7176 bicycle-or-wheelchair
  safety certification mark -- this actor is never the certification
...
(defn register-maintenance
  ...
  builds the RECORD a plant coordinator would keep. `bikemfg.governor`
...
(defn register-shipment
  ...
  outbound bicycle/invalid-carriage shipment against a verified,
  ...
  keep. `bikemfg.governor` independently re-verifies the shipment's
