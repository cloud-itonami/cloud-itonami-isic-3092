(ns bikemfg.operation-test
  "Smoke tests for the compiled BikeOperationActor graph itself (build
  + one happy path per op). The governor's full rule contract (HARD
  holds, escalation, phase gating) is exercised in
  `bikemfg.governor-contract-test`; the Store contract in
  `bikemfg.store-contract-test`."
...
(deftest test-actor-builds
  (testing "BikeOperationActor can be built with a store"
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "welder-001" :severity :moderate :description "frame weld anomaly"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))
