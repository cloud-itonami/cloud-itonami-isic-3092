(ns bikemfg.governor-contract-test
  ...
  equipment operations... does NOT self-issue an ISO 4210/ISO 7176
  bicycle-or-wheelchair safety certification') implemented faithfully.
  The single invariant under test:

    BikeAdvisor never schedules maintenance, flags a safety concern,
    or coordinates a shipment the Bicycle & Mobility Device Plant
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:product-category :road-bicycle}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :road-bicycle (:product-category (store/batch db "batch-001"))) "SSoT actually updated")
...
(deftest certification-authority-is-held-and-permanently-blocked
  (testing "a proposal that sets :issue-certification? true -> HOLD, PERMANENT, never reaches request-approval -- this actor is never the ISO 4210/ISO 7176 certification authority"
    (let [[db actor] (fresh)
...
(deftest invalid-weight-capacity-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10b" {:op :log-production-batch :effect :propose :subject "batch-001"
                                   :patch {:weight-capacity-kg 9999.0}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-weight-capacity} (-> (store/ledger db) last :basis)))
    (is (not= 9999.0 (:weight-capacity-kg (store/batch db "batch-001"))) "fabricated weight-capacity never lands in the SSoT")))
...
(deftest safety-concern-approval-rejected-leaves-no-record-only-a-hold-fact
  (let [[db actor] (fresh)
        _ (exec-op actor "t13" {:op :flag-safety-concern :effect :propose :subject "concern-2"
                                :value {:equipment-id "welder-001" :severity :low :description "y"}}
                   coordinator)
        r (reject! actor "t13")]
